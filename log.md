# 100 Days Of Code - Log

<!-- ### Day {NUM}: {MONTH} {DAY}, {YEAR}

**Today's Progress**: Day 25 of the #100DaysOfCode completed. Used #Python #pandas to work with data and made a simple educational game to learn the States. 
 

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [Find the Longest Word in a String](https://www.freecodecamp.com/challenges/find-the-longest-word-in-a-string) -->

### Day 25: January 25, 2021

**Today's Progress**: Used #Python #pandas to work with data and made a simple educational game to learn the States.

**Thoughts** Today was a little challenging because I had to learn a new module and its documentation, but the benefits of using pandas seem to outweigh any growing pains. 

**Link(s) to work**
1. [us-states-game](day-25/us-states-game/us-states-game.py)
2. [day-25-twitter-checkup](<blockquote class="twitter-tweet"><p lang="en" dir="ltr">Day 25 of the <a href="https://twitter.com/hashtag/100DaysOfCode?src=hash&amp;ref_src=twsrc%5Etfw">#100DaysOfCode</a> completed. Used <a href="https://twitter.com/hashtag/Python?src=hash&amp;ref_src=twsrc%5Etfw">#Python</a> <a href="https://twitter.com/hashtag/pandas?src=hash&amp;ref_src=twsrc%5Etfw">#pandas</a> to work with data and made a simple educational game to learn the States. Thanks to <a href="https://twitter.com/yu_angela?ref_src=twsrc%5Etfw">@yu_angela</a> and <a href="https://twitter.com/LondonAppBrewer?ref_src=twsrc%5Etfw">@LondonAppBrewer</a> <br>Check out my code on <a href="https://twitter.com/github?ref_src=twsrc%5Etfw">@github</a> at <a href="https://t.co/sJOsm4k1Kr">https://t.co/sJOsm4k1Kr</a>. <a href="https://t.co/xPumEMMwEX">pic.twitter.com/xPumEMMwEX</a></p>&mdash; Stanislav Fatkhutdinov (@SFatkhutdinov) <a href="https://twitter.com/SFatkhutdinov/status/1353807929363881984?ref_src=twsrc%5Etfw">January 25, 2021</a></blockquote> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>)

### Day 24: January 24, 2021

**Today's Progress**: Mail Merger project using 

**Thoughts** I was cutting it kind of close, got done by midnight. But I think I understand the paths and the differences between absolute and relative paths. 

**Link(s) to work**
1. [mail-merger-project](day-24/mail-merger-project.py)

### Day 23: January 23, 2021

**Today's Progress**: Turtle crossing game 

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [turtle-crossing](day-23/turtle-crossing.py)

### Day 22: January 22, 2021

**Today's Progress**: A classic pong game for two players

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [pong-game](day-22/pong-game.py)

### Day 21: January 21, 2021

**Today's Progress**: Improved the snake game from yesterday

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [snake](day-20/main.py)

### Day 20: January 20, 2021

**Today's Progress**: A game of snake

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [snake](day-20/main.py)

### Day 19: January 19, 2021

**Today's Progress**: Made a turtle race betting game

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [turtle-race](day-19/turtle-race.py)

### Day 18: January 18, 2021

**Today's Progress**: Hirst-style painting using the Python turtle module

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [hirst-painting](day-18/hirst-painting.py)

### Day 17: January 17, 2021

**Today's Progress**: Quiz game that uses OOP

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [quiz-game](day-17/quiz-game.py)

### Day 16: January 16, 2021

**Today's Progress**: Object Oriented Programing approach to making a coffee machine.

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [coffee-maker-machine-oop](day-16/coffee-maker-machine-oop.py)

### Day 15: January 15, 2021

**Today's Progress**: Made a coffee machine that accepts coins and tracks the resources used.

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [coffee-machine](day-15/coffee-machine.py)

### Day 14: January 14, 2021

**Today's Progress**: Higher-Lower game. Determine which answer is correct.

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [higher-lower-game](day-14/higher-lower-game.py)

### Day 13: January 13, 2021

**Today's Progress**: FizzBuzz program using conditional logic

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [fizz-buzz](day-13/fizz-buzz.py)

### Day 12: January 12, 2021

**Today's Progress**: Used a random module to make a number-guessing game

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [number-guessing-game](day-12/number-guessing-game.py)

### Day 11: January 11, 2021

**Today's Progress**: A game of blackjack

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [blackjack](day-11/blackjack.py)

### Day 10: January 10, 2021

**Today's Progress**: Made a basic calculator using functions.

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [calculator](day-10/calculator.py)

### Day 9: January 9, 2021

**Today's Progress**: Created a blind auction program

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [blind-auction](day-09/blind-auction.py)

### Day 8: January 8, 2021

**Today's Progress**: Dove into cryptology by making a simple caesar cipher

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [caesar-cipher](day-08/caesar-cipher.py)
### Day 7: January 7, 2021

**Today's Progress**: Wrote a simple game of Hangman

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [hangman-game](day-07/hangman-game.py)

### Day 6: January 6, 2021

**Today's Progress**: Solved a maze by applying logical conditions

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [maze](day-06/maze.py)

### Day 5: January 5, 2021

**Today's Progress**: Created a simple password-generator

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [password-generator](day-05/password-generator.py)

### Day 4: January 4, 2021

**Today's Progress**: Made a simple Rock Paper Scissors game

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [rock-paper-scissors](day-04/rock-paper-scissors.py)

### Day 3: January 3, 2021

**Today's Progress**: Created a treasure-island game

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [treasure-island](day-03/treasure-island.py)

### Day 2: January 2, 2021

**Today's Progress**: Created a tip-calculator

**Thoughts** I am restoring the log so I don't remember my thoughts for this day. 

**Link(s) to work**
1. [tip-calculator](day-02/tip-calculator.py)

### Day 1: January 1, 2021

**Today's Progress**: Started the challenge. Created the band-name-generator.

**Thoughts** I am restoring the log so I don't remember my thoughts for this day.

**Link(s) to work**
1. [Band-Name-Generator](day-01/band-name-generator.py)
